...
spawn_time =      72 * random_range(1, 1.5); // 90
drenneth_1_time = 55 * random_range(1, 1.5); // 45
drenneth_2_time = 25 * random_range(1, 1.5); // 30
ash_1_time =      60 * random_range(1, 1.5); // 45
ash_2_time =      35 * random_range(1, 1.5); // 55
max_delay = irandom_range(5, 12);
  ...
  if (time_source_get_state(drenneth_ts) != time_source_state_active)
  {
    time_source_reconfigure(drenneth_ts, irandom_range(drenneth_1_time, drenneth_1_time + max_delay), time_source_units_seconds, function() {
      if (
        ...
        audio_play_sound_on(o_snd_manager.emitter, sfx_tile_break, false, 100, .6);
        drenneth = drenneth_2;
        global.drenneth = 2;
      }
    });
